...
public enum ManaCost {
    /// Glyph used for the tap symbol `{T}`. The Unicode clockwise arrow curls the
    /// right way but sits 180° from the printed card symbol, so the renderer
    /// rotates it half a turn — see `ManaPipDisc`.
    public static let tapGlyph = "↻"
    ...
    private static func half(for token: Substring) -> ManaPip.Half {
        ...
        case "T": return ManaPip.Half(glyph: tapGlyph, identity: .colorless)
